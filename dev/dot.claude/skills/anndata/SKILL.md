...
- **Backed sparse (`_CSRDataset`/`_CSCDataset`):** returned by `read_h5ad(..., backed="r")`.
  ...
  must be sliced to materialize. h5py is **not thread-safe** → backed reads are never threaded;
  the single-file converter parallelises them across **processes** instead (each worker opens
  its own handle via `matrix.group.file.filename` / `matrix.group.name`). DataScale reads
  `indptr` via `matrix.group["indptr"]` for backed input; verify that path in `sparse_dataset.py`.
...
- **Backed slicing** (`src/anndata/_core/sparse_dataset.py`):
  ...
  minor axis must scan all major vectors. h5py is not thread-safe → backed reads are never
  threaded, but are parallelised across **processes** (see CLAUDE.md parallelism model).
