...
description: >-
  ...
  choosing chunk shape, sharding, consolidated metadata,
...
- **Read concurrency = two knobs, set BOTH.** `async.concurrency` only *dispatches* chunk
  fetches (a semaphore, no CPU work); `threading.max_workers` sizes the pool that runs
  **decode** (`BloscCodec` → `asyncio.to_thread`, verify in `codecs/blosc.py` + `core/sync.py`).
  Decompression is the dominant cost of a warm dense read and releases the GIL, so it scales
  across cores — but only up to `max_workers`. Raising `async.concurrency` alone ≈ 1 core;
  `async.concurrency=1` starves the pool regardless of workers. Set both:
  `zarr.config.set({"async.concurrency": ~64, "threading.max_workers": <physical cores>})`.
  zarr assembles the result on one event-loop thread, so effective cores plateau below
  physical; a hand-rolled per-chunk reader (read→decode→place per thread) can beat it on
  simple contiguous dense slices by parallelising placement too — measure first.
- **dask on the read path — only when needed.** dask does *not* speed up an in-process read
  over zarr's own threads (wrapping a read in the threaded scheduler with zarr pinned to 1 is
  *slower* — all tasks funnel through one shared zarr event loop). Reach for dask only to (a)
  **stream** data that exceeds host/GPU memory block-by-block, or (b) feed **distributed
  workers/GPUs that consume blocks in place** (e.g. `dask-cuda` GPU pipelines). Don't
  `.compute()` a big dense block to the client (gather tax dominates). Under dask, turn the
  two zarr knobs *down* per worker — they multiply (`workers × threads × concurrency × max_workers`).
