...
def _stream_sorted_store(
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="convert_to_zarr_sort_", dir=str(output_path.parent)))
    try:
        ...
        store, finalize = open_output_store(
            output_path, cfg, commit_message=f"convert-to-zarr convert (sorted) → {output_path.name}",
        )
