...
def _self_serve_subset(g, **keys):
    ...
    datascale, no datascale index. Because the store is physically sorted by the keys, the
...
def test_sort_writes_valid_anndata_no_index(tmp_path: Path) -> None:
    ...
    # NO datascale-specific index is written — the store is a plain sorted AnnData.
    assert "datascale_sort_index" not in adata.uns
...
def test_sort_dense_writes_contiguous_ranges(tmp_path: Path) -> None:
    ...
    (stock zarr, no datascale, no index)."""
    _labelled_h5ad(tmp_path / "in.h5ad")
