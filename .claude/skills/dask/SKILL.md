...
- **Scheduler rules (mandatory, not tuning):**
  ...
  - Backed HDF5 (`_CSRDataset`) → `scheduler="synchronous"`, 1 worker. **h5py is not
    thread-safe.** Never thread a backed read.
- **`da.from_delayed`** is used (not `from_array`) for backed sparse input because backed
  datasets have no ndim / array protocol. Each delayed block must declare correct `shape` and
  `dtype`.
